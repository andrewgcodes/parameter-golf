"""
Comprehensive Parameter Golf submission combining all proven techniques:
- MLP 3x multiplier
- Int8 quantization + zlib-9 compression (competition standard)
- SmearGate (learned adjacent token blending)
- BigramHash embedding (hash-based bigram context)
...
- NTK-aware RoPE scaling for eval
- Warmdown LR decay for quantization-friendly weights
...
def eval_val(args, model, rank, world_size, device, grad_accum_steps, val_tokens,
             base_bytes_lut, has_leading_space_lut, is_boundary_token_lut, seq_len_override=0):
...
def eval_val_sliding(logits_fn, rank, world_size, device, val_tokens,
    ...
    while p + seq_len <= total:
        ...
        windows.append((p, s))
        p += stride
    # Score final partial window
    if p < total and total >= seq_len:
        last_start = total - seq_len
        if not windows or windows[-1][0] != last_start:
            s = seq_len - (total - p)
            windows.append((last_start, s))
...
def quantize_float_tensor(t: Tensor, bits: int = 8) -> tuple[Tensor, Tensor]:
    """Per-row quantization for 2D, per-tensor for others. bits=6 stores int6 values in int8."""
...
FP16_TIED_EMBED = bool(int(os.environ.get("FP16_TIED_EMBED", "1")))
LATE_K_LAYERS = int(os.environ.get("LATE_K_LAYERS", 1))

def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    """Quantization with int6/int8 + FP16 tied embed + Late-K passthrough."""
    ...
    for name, tensor in state_dict.items():
        ...
        # FP16 passthrough for tied embedding (reduces quant damage to dual-role tensor)
        if FP16_TIED_EMBED and name == "tok_emb.weight":
        ...
        # Late-K passthrough: keep last N layers' key weights in fp16
        if LATE_K_LAYERS > 0 and name.endswith("c_k.weight") and any(
        ...
        # Quantize with configurable bit width (int6 stores in int8, zlib compresses well)
...
class BigramHashEmbedding(nn.Module):
    """Hash-based bigram embedding for token pair context."""
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
                 qk_gain_init: float, train_seq_len: int = 1024):
        super().__init__()
        if dim % num_heads != 0:
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int,
                 ...
                 train_seq_len: int = 1024, use_smeargate: bool = False,
                 use_bigram_hash: bool = False, bigram_vocab_size: int = 4096,
                 bigram_dim: int = 128):
        super().__init__()
        ...
        # Optional BigramHash
        self.bigram_hash = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim) if use_bigram_hash else None
    ...
    def _init_weights(self) -> None:
        ...
        # Orthogonal init with muP scaling for linear layers
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        train_seq_len=args.train_seq_len, use_smeargate=args.use_smeargate,
        use_bigram_hash=args.use_bigram_hash, bigram_vocab_size=args.bigram_vocab_size,
    ...
    if base_model.smeargate is not None:
        for p in base_model.smeargate.parameters():
    ...
    log0(f"config: layers={args.num_layers} dim={args.model_dim} heads={args.num_heads} kv_heads={args.num_kv_heads} mlp_mult={args.mlp_mult}")
    log0(f"smeargate={args.use_smeargate} bigram_hash={args.use_bigram_hash} swa={args.swa_enabled}")
    log0(f"rope_base={args.rope_base} seq_len={args.train_seq_len} batch_tokens={args.train_batch_tokens}")
    log0(f"matrix_lr={args.matrix_lr} scalar_lr={args.scalar_lr} muon_wd={args.muon_wd} grad_clip={args.grad_clip_norm}")
    log0(f"warmdown={args.warmdown_iters} eval_stride={args.eval_stride}")
    ...
    while True:
        ...
        # Decoupled weight decay for Muon params
        if args.muon_wd > 0:
            with torch.no_grad():
        ...
        # SWA: accumulate checkpoint averages
        if args.swa_enabled and max_wallclock_ms is not None:
            approx_frac = (training_time_ms + 1000.0 * (time.perf_counter() - t0)) / max_wallclock_ms
            if approx_frac >= args.swa_start_frac:
                ...
                if step % args.swa_every == 0:
                    ...
                    if swa_state is None:
                        swa_state = {k: v.detach().cpu().clone().float() for k, v in sd.items()}
                        swa_count = 1
                    else:
                        for k in swa_state:
    ...
    # Apply SWA if we have accumulated checkpoints
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        log0(f"Applying SWA with {swa_count} checkpoints")
        ...
        swa_count += 1
        # Average
        averaged = {k: (v / swa_count).to(sd[k].dtype) for k, v in swa_state.items()}
        base_model.load_state_dict(averaged, strict=True)
        # Evaluate SWA model
        val_loss, val_bpb = eval_val(args, model, rank, world_size, device, grad_accum_steps,
                                     val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
        log0(f"SWA val_loss:{val_loss:.4f} val_bpb:{val_bpb:.4f}")
    ...
    # Quantize with int8 + zlib-9 (competition standard)
    ...
    if master_process:
        ...
        log0(f"Serialized model int8+zlib-9: {quant_file_bytes} bytes "
        ...
        if quant_file_bytes + code_bytes > 16_000_000:
            log0(f"WARNING: Submission exceeds 16MB limit!")

    # Roundtrip: decompress, dequantize, evaluate
    if distributed:
        dist.barrier()
    ...
    if eval_sl != args.train_seq_len:
        ...
        log0(f"eval_seq_len:{eval_sl} (NTK-scaled RoPE)")
    else:
    ...
    if eval_stride > 0:
        ...
        log0("forward_logits compiled, starting sliding window eval...")
        q_val_loss, q_val_bpb = eval_val_sliding(
            compiled_logits, rank, world_size, device, val_tokens_eval,
            base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            eval_sl, eval_stride, eval_batch_seqs,
        )
    else:
        q_val_loss, q_val_bpb = eval_val(args, model, rank, world_size, device, grad_accum_steps,
                                          val_tokens_eval, base_bytes_lut, has_leading_space_lut,
                                          is_boundary_token_lut, seq_len_override=eval_sl)
