"""
Maximalist Batch + Curriculum Learning submission for parameter-golf.
Stacks: int6 quant, MLP 3x, 10L, sliding window eval, FP16 embed,
aggressive warmdown, overtone init, Muon WD, large batch, curriculum seq len.
...
class Hyperparameters:
    ...
    # Model shape: 10 layers, MLP 3x via mlp_hidden
    ...
    # Optimizer hyperparameters (tuned for larger batch + aggressive warmdown)
...
# -----------------------------
# POST-TRAINING QUANTIZATION (int6/int8)
# -----------------------------
...
def quantize_state_dict(state_dict: dict[str, Tensor], quant_bits: int = 8):
    ...
    for name, tensor in state_dict.items():
        ...
        # FP16 passthrough for tied embeddings (reduces quant damage on shared weight)
        if "tok_emb" in name:
        ...
        # Use int6 for block params, int8 for everything else
        bits = quant_bits if ("blocks." in name) else 8
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base, qk_gain_init, mlp_hidden=mlp_hidden,
                )
...
def main() -> None:
    ...
    log0(f"curriculum_enabled:{args.curriculum_enabled} "
         f"curriculum_start_seq:{args.curriculum_start_seq} "
         f"curriculum_mid_seq:{args.curriculum_mid_seq} "
         f"phase1_frac:{args.curriculum_phase1_frac} "
         f"phase2_frac:{args.curriculum_phase2_frac}")
    log0(f"quant_bits:{args.quant_bits} muon_weight_decay:{args.muon_weight_decay}")
    ...
    def get_curriculum_seq_len(elapsed_ms: float) -> int:
        """Return current sequence length based on curriculum schedule."""
        if not args.curriculum_enabled or max_wallclock_ms is None:
            return args.train_seq_len
        frac = elapsed_ms / max_wallclock_ms
        if frac < args.curriculum_phase1_frac:
            return args.curriculum_start_seq
        elif frac < args.curriculum_phase1_frac + args.curriculum_phase2_frac:
            return args.curriculum_mid_seq
        else:
            return args.train_seq_len

    # Warmup primes compiled paths, then restores initial state
    ...
    while True:
        ...
        if last_step:
            if stop_after_step is not None and step < args.iterations:
                log0(
                    ...
                    f"step:{step}/{args.iterations} tokens_seen:{total_tokens_seen}"
                )
            break

        elapsed_ms = training_time_ms + 1000.0 * (time.perf_counter() - t0)
        scale = lr_mul(step, elapsed_ms)
        curr_seq_len = get_curriculum_seq_len(elapsed_ms)
        ...
        if should_log_train:
            log0(
                ...
                f"seq_len:{curr_seq_len} tokens_seen:{total_tokens_seen}"
            )
