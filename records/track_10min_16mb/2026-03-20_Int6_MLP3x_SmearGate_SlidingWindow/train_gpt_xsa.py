...
class Hyperparameters:
    ...
    # Partial RoPE: apply rotary embeddings to only rope_dims of head_dim (0 = full head_dim)
    rope_dims = int(os.environ.get("ROPE_DIMS", 0))
    # LN Scale: scale RMSNorm outputs by 1/sqrt(layer_idx+1) to damp deeper layers
    ln_scale = bool(int(os.environ.get("LN_SCALE", "0")))
    # Late QAT: enable int6 fake-quantization only in final ~4% of training (lr_scale < threshold)
